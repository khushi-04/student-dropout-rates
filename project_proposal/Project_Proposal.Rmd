---
title: 'Group 6 Project Proposal'
author: "Group 6"
output:
  pdf_document:
    toc: yes
...
#Note:
# Once we finalize  our research questions, we can remove any variables 
# that won't be useful to answer the questions
```
...
plot1 <- ggplot(data = students_df)+
  geom_col(aes(x = Target, y = Admission.grade ), color = "lightblue")+
  labs(title = "Target vs. Admission Grade", x = "Target Response", 
       y = "Admission Grade")
plot1 <- ggplot(students_df, aes(x=Admission.grade, fill=Target)) +
  geom_histogram(binwidth=5, alpha=0.5, position="identity") +
  labs(title="Distribution of Admission Grades by Target", x="Admission Grades", y="Count") +
  theme_minimal()

# Distribution of Target by Gender Jitterplot
plot2 <- ggplot(data = students_df)+
  geom_jitter(aes(x = Target, y = Gender.labeled))+
  labs(title = "Target vs. Gender", x = "Target Response", y = "Gender")

# Age vs Target violin plot
plot3 <- ggplot(data = students_df, aes(x = Target, y = Age))+
  geom_violin()+
...
plot4 <- ggplot(data = students_df, aes(x = Age))+
  ...
  labs(title = "Distribution of Age During Course Enrollment", x = "Age at Enrollment")+
  theme_minimal()+
  theme_classic()+
